...
public class GroupApp {
	...
	public static void main(String[] args) throws Exception{
		...
		//1.4 TODO 排序、分组（和分区有区别，分区指的是REDUCE根据业务需求输出到不同的文件中
		//，分组指的是MAP输出分组，即K2,V2根据规则分组，这里自定义了分组类，以相同的K2为一组输出到REDUCE方法）
	 ...
	 * 
	 * 为什么自定义该类
	 * 业务要求分组是按照第一列分，但是NEWK2的比较规则决定了不能按照第一列分，只能自定义分组比较类
	 *
	 */
	static class MyGroupingComparator implements RawComparator<NewK2>{
		...
		/**
		 * @param b1 第一个参数比较的字节数组
		 * @param s1 第一个参与比较的字节数组的起始位置
		 * @param l1 第一个参与比较字节数组的偏移量
		 * @param b2 第2个参数比较的字节数组
		 * @param s2 第2个参与比较的字节数组的起始位置
		 * @param l2 第2个参与比较字节数组的偏移量
		 */
		@Override
